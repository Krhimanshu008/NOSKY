...
async function runBenchmark() {
  ...
  try {
    ...
    const articlesUnbounded = await collection
      .find({})
      .sort({ createdAt: -1 })
      .toArray();
    ...
    console.log(
      `Unbounded query returned ${articlesUnbounded.length} articles in ${timeUnboundedMs.toFixed(2)} ms`,
    );
    ...
    const articlesLimited = await collection
      .find({})
      .sort({ createdAt: -1 })
      .limit(100)
      .toArray();
    ...
    console.log(
      `Limited query returned ${articlesLimited.length} articles in ${timeLimitedMs.toFixed(2)} ms`,
    );
    console.log(
      `Difference: ${(timeUnboundedMs - timeLimitedMs).toFixed(2)} ms`,
    );
